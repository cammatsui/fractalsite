    ...
    private setCells(value: boolean) {
        if (this.is2D) {
            for (var i = 0; i < this.matrix2D.length; i++) {
                for (var j = 0; j < this.matrix2D[0].length; j++) this.matrix2D[i][j] = value;
            }
        } else {
            for (var i = 0; i < this.matrix3D.length; i++) {
                for (var j = 0; j < this.matrix3D[0].length; j++) {
                    for (var k = 0; k < this.matrix3D[0][0].length; k++) this.matrix3D[i][j][k] = value;
                }
            }
        }
    } // setCells ()
    ...
    private draw3DCells(row: number, col: number) {
        ...
        for (var i = 3; i >= 0; i--) {
            ...
            this.ctx.rect(row*this.inc + (i+0.5)*inc3D + this.labelBufferWidth, 
                col*this.inc + (i+0.5)*inc3D + this.labelBufferWidth, 2*inc3D, 2*inc3D);
